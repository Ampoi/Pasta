...
const getMainCode = async (
  ...
  variableIDs.trigger = {}
  for (const output of triggerOutputs) {
  ...
  for await (const layer of layers) {
    for await (const nodeID of layer) {
      ...
      if (node.inputs) {
        for (const [portID, port] of Object.entries(node.inputs)) {
      ...
      mainCodeLines.push(`${Object.keys(outputVariables).length > 0
          ? `const { ${Object.entries(outputVariables).map(([outputID, variableID]) => `${outputID}: ${variableID}`).join(", ")
          } } = `
          : ""
        }${blockID}(${Object.entries(input).map(([, value]) => value).join(", ")
        })`)
    }
  }

  return {
    mainCode: `const main = (${Object.keys(variableIDs.trigger).length == 0 ? "" : `{${Object.entries(variableIDs.trigger).map(([outputID, variableName]) => `${outputID}: ${variableName}`).join(", ")
        }}`
